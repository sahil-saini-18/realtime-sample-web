...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    super();
    this.defaultUrl = 'wss://ai-world-test.openai.azure.com/openai/realtime';
  ...
  async connect({ model } = { model: 'gpt-4o-realtime-preview-1001' }) {
    ...
    if (globalThis.WebSocket) {
      ...
      const ws = new WebSocket(
        'wss://ai-world-test.openai.azure.com/openai/realtime?api-version=2024-10-01-preview&deployment=gpt-4o-realtime-preview-1001',
        [],
        {
  ...
  send(eventName, data) {
    if (!this.isConnected()) {
      throw new Error(` my RealtimeAPI is not connected`);
    }
    data = data || {};
